...
def test_generate_delta_map():
    ...
    generate_delta_map(map_paths, 'original_firstorder_Entropy', './outputs/Patient_01_F1_F5/')

    delta_map = np.load('./outputs/Patient_01_F1_F5/original_firstorder_Entropy.npy')
    fig = plt.figure(figsize=(10, 5))
    plt.imshow(delta_map[:, :, 15], cmap='viridis'), plt.colorbar(), plt.title('Entropy Delta radiomic map'), plt.show()

def test_cluster_delta_map(): 
    '''Function that aims to test the clustering of a delta-radiomic map. ''' 

    # TODO: keep the same size as the original image 
    ...
    plt.subplot(121), plt.imshow(clustered_map[:, :, 15], cmap='viridis'), plt.colorbar(), plt.title('Clustered Entropy Delta radiomic map')
    plt.subplot(122), plt.imshow(delta_map[:, :, 15], cmap='viridis'), plt.colorbar(), plt.title('Clustered Entropy Delta radiomic map')
    plt.show()
