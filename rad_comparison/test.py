...
results_dict = json.load(open('json_results/all_results_cv.json'))
...
results = pd.read_json('json_results/all_results_cv.json') 


top_results = af.get_best_results(results, delta_rad_tables, feat_sel_algo_list, pred_algo_list, 'Récidive Locale', metric='roc_auc', k = 1)

nice_tables = ['F1','F2', 'F3', 'F4', 'F5', 'F1/F5', 'F1/F2', 'F1/F3', 'F2/F3', 'F1/F4']

af.compare_roc_auc(top_results, 'Récidive Locale', nice_tables, cval=True)