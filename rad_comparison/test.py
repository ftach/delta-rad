...
results = pd.read_json('json_results/result_gie.json') 


top_results = af.get_best_results(results, delta_rad_tables, feat_sel_algo_list, pred_algo_list, 'Décès', metric='roc_auc', k = 1)

# nice_tables = ['F1','F2', 'F3', 'F4', 'F5', 'F1/F5', 'F1/F2', 'F1/F3', 'F2/F3', 'F1/F4']
nice_tables = ['1 month', 'Simu', 'Delta']

af.compare_roc_auc(top_results, 'Décès', nice_tables, cval=True)